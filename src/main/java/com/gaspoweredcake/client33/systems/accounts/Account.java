...
import com.gaspoweredcake.client33.mixin.SkinManagerAccessor;
...
public abstract class Account<T extends Account<?>> implements ISerializable<T> {
    ...
    public static void setSession(User session) {
        ...
        mca.client33$setPlayerSocialManager(new PlayerSocialManager(mc, apiService, friendsService, remoteFriendListUpdateHandler));
        mca.client33$setProfileKeyPairManager(ProfileKeyPairManager.create(apiService, session, mc.gameDirectory.toPath()));
        mca.client33$setReportingContext(ReportingContext.create(ReportEnvironment.local(), apiService));
        mca.client33$setProfileFuture(CompletableFuture.supplyAsync(() -> mc.services().sessionService().fetchProfile(mc.getUser().getProfileId(), true), Util.ioPool()));
    }
    ...
    public static void applyLoginEnvironment(YggdrasilAuthenticationService authService) {
        ...
        SkinManager.TextureCache skinCache = ((SkinManagerAccessor) mc.getSkinManager()).client33$getSkinTextures();
        Path skinCachePath = ((FileCacheAccessor) skinCache).client33$getRoot();
        mca.client33$setServices(Services.create(authService, mc.gameDirectory));
        mca.client33$setSkinManager(new SkinManager(skinCachePath, mc.services(), new SkinTextureDownloader(mc.getProxy(), mc.getTextureManager(), mc), mc));
    }
