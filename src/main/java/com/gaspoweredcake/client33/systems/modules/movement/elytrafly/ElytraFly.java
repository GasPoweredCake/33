...
import com.gaspoweredcake.client33.mixin.BlockBehaviourAccessor;
...
public class ElytraFly extends Module {
    ...
    private void onPlayerMove(PlayerMoveEvent event) {
        ...
        if (mc.player.isFallFlying()) {
            ...
            if (dontGoIntoUnloadedChunks.get()) {
                if (mc.level.getChunkSource().hasChunk(chunkX, chunkZ)) {
                    if (flightMode.get() != ElytraFlightModes.Bounce)
                        ((IVec3) event.movement).client33$set(currentMode.velX, currentMode.velY, currentMode.velZ);
                } else {
                    currentMode.zeroAcceleration();
                    ((IVec3) event.movement).client33$set(0, currentMode.velY, 0);
                }
        ...
        if (autoHover.get() && mc.player.input.keyPresses.shift() && !Modules.get().get(Freecam.class).isActive() && mc.player.isFallFlying() && flightMode.get() != ElytraFlightModes.Bounce) {
            ...
            final boolean underCollidable = ((BlockBehaviourAccessor) under).client33$isHasCollision() || !underState.getFluidState().isEmpty();
            final boolean under2Collidable = ((BlockBehaviourAccessor) under2).client33$isHasCollision() || !under2State.getFluidState().isEmpty();

            if (!underCollidable && under2Collidable) {
                ((IVec3) event.movement).client33$set(event.movement.x, -0.1f, event.movement.z);
            ...
            if (underCollidable) {
                ((IVec3) event.movement).client33$set(event.movement.x, -0.03f, event.movement.z);
                ...
                if (mc.player.position().y <= mc.player.blockPosition().below().getY() + 1.34f) {
                    ((IVec3) event.movement).client33$set(event.movement.x, 0, event.movement.z);
