...
import com.gaspoweredcake.client33.events.entity.player.InteractEntityEvent;
...
import com.gaspoweredcake.client33.mixin.AbstractContainerScreenAccessor;
...
public class InventoryTweaks extends Module {
    ...
    private boolean sort() {
        ...
        Slot focusedSlot = ((AbstractContainerScreenAccessor) screen).client33$getHoveredSlot();
        ...
        for (int i = start; i < end; i++) {
            ...
            if (sleep > 0) {
                try {
                    ...
                    Client33.LOG.error("Error when sleeping the slot mover", e);
                }
            }
    ...
    public void steal(AbstractContainerMenu handler) {
        Client33Executor.execute(() -> moveSlots(handler, 0, SlotUtils.indexToId(SlotUtils.MAIN_START), true));
    }

    public void dump(AbstractContainerMenu handler) {
        int playerInvOffset = SlotUtils.indexToId(SlotUtils.MAIN_START);
        Client33Executor.execute(() -> moveSlots(handler, playerInvOffset, playerInvOffset + 4 * 9, false));
    }
