...
import com.gaspoweredcake.client33.events.render.RenderItemEntityEvent;
import com.gaspoweredcake.client33.mixin.ItemStackRenderStateAccessor;
import com.gaspoweredcake.client33.mixin.LayerRenderStateAccessor;
...
public class ItemPhysics extends Module {
    ...
    private void onRenderItemEntity(RenderItemEntityEvent event) {
        ...
        for (int i = 0; i < ((ItemStackRenderStateAccessor) event.renderState.item).client33$getActiveLayerCount(); i++) {
            ItemStackRenderState.LayerRenderState layer = ((ItemStackRenderStateAccessor) event.renderState.item).client33$getLayers()[i];
            ModelInfo info = getInfo(layer.prepareQuadList());

            matrices.pushPose();
            applyTransformation(matrices, ((LayerRenderStateAccessor) layer).client33$getTransform());
