...
import com.gaspoweredcake.client33.mixin.DirectionAccessor;
...
public class AutoWasp extends Module {
    ...
    private void onMove(PlayerMoveEvent event) {
        ...
        if (avoidLanding.get()) {
            ...
            for (Direction dir : DirectionAccessor.client33$getHorizontal()) {
                BlockPos pos = BlockPos.ofFloored(targetPos.offset(dir, d).offset(dir.rotateYClockwise(), d)).down();
                if (((AbstractBlockAccessor) mc.world.getBlockState(pos).getBlock()).client33$isCollidable() && Math.abs(targetPos.getY() - (pos.getY() + 1)) <= 0.25) {
        ...
        ((IVec3d) event.movement).client33$set(xVel, yVel, zVel);
    }
