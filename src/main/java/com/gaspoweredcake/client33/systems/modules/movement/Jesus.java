...
import com.gaspoweredcake.client33.events.entity.player.CanWalkOnFluidEvent;
...
import com.gaspoweredcake.client33.events.world.CollisionShapeEvent;
import com.gaspoweredcake.client33.events.world.TickEvent;
import com.gaspoweredcake.client33.mixin.EntityMixin;
...
import com.gaspoweredcake.client33.systems.modules.movement.speed.modes.Strafe;
...
public class Jesus extends Module {
    ...
    private void onTick(TickEvent.Post event) {
        ...
        if (bubbleColumn) {
            if (mc.options.jumpKey.isPressed() && movingEntity.getVelocity().getY() < 0.11) ((IVec3d) movingEntity.getVelocity()).client33$setY(0.11);
            return;
        }

        // Move up
        if (movingEntity.isTouchingWater() || movingEntity.isInLava()) {
            ((IVec3d) movingEntity.getVelocity()).client33$setY(0.11);
            ascending = 0;
            return;
        }
        ...
        if (ascending == 0) ((IVec3d) movingEntity.getVelocity()).client33$setY(0.11);
        else if (ascending == 1 && (blockBelowState.getBlock() == Blocks.WATER || blockBelowState.getBlock() == Blocks.LAVA || waterLogged))
            ((IVec3d) movingEntity.getVelocity()).client33$setY(0);
    ...
    private void onSendPacket(PacketEvent.Send event) {
        ...
        ((PlayerMoveC2SPacketAccessor) packet).client33$setOnGround(false);
        if (!mc.player.isOnGround() || !packet.changesPosition()) return;

        ((PlayerMoveC2SPacketAccessor) packet).client33$setY(packet.getY(0) - (0.02 + (0.0001 * swimmingTicks)));
    }
    ...
    private void onMoveEvent(PlayerMoveEvent event) {
        ...
        if (++swimmingTicks < 15) {
            if (mc.player.isOnGround()) {
                ...
                ((IVec3d) event.movement).client33$setXZ(vel.x, vel.y);
            }
        ...
        swimmingTicks = 0;
        if (slowDown.get()) ((IVec3d) event.movement).client33$setXZ(0, 0);
        ((IVec3d) mc.player.getVelocity()).client33$setY(0.08);
    }
