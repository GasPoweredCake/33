...
public abstract class System<T> implements ISerializable<T> {
    ...
    public System(String name) {
        ...
        if (name != null) {
            this.file = new File(Client33.FOLDER, name + ".nbt");
    ...
    public void save(File folder) {
        ...
        try {
            File tempFile = File.createTempFile(Client33.MOD_ID, file.getName());
            ...
            Client33.LOG.error("Error saving {}. Possibly corrupted?", this.name, e);
        }
    }
        ...
        try {
            ...
            if (file.exists()) {
                try {
                    ...
                    Client33.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
                    Client33.LOG.info("Saved settings backup to '{}'.", backup);
                }
            }
        } catch (IOException e) {
            Client33.LOG.error("Error loading {}. Possibly corrupted?", this.name, e);
