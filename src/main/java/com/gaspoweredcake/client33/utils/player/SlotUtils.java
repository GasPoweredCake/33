...
import com.gaspoweredcake.client33.mixin.ItemGroupsAccessor;
import com.gaspoweredcake.client33.mixin.MountScreenHandlerAccessor;
...
public class SlotUtils {
    ...
    private static int creativeInventory(int i) {
        if (CreativeInventoryScreenAccessor.client33$getSelectedTab() != Registries.ITEM_GROUP.get(ItemGroupsAccessor.client33$getInventory()))
            return -1;
    ...
    private static int horse(ScreenHandler handler, int i) {
        LivingEntity entity = ((MountScreenHandlerAccessor) handler).client33$getMount();
