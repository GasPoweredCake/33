...
public class InventorySorter {
    ...
    private void generateActions() {
        ...
        for (Slot slot : screen.getScreenHandler().slots) {
            if (getInvPart(slot) == originInvPart) slots.add(new MySlot(((ISlot) slot).client33$getId(), slot.getStack()));
        }
    ...
    private InvPart getInvPart(Slot slot) {
        int i = ((ISlot) slot).client33$getIndex();

        if (slot.inventory instanceof PlayerInventory && (!(screen instanceof CreativeInventoryScreen) || ((ISlot) slot).client33$getId() > 8)) {
