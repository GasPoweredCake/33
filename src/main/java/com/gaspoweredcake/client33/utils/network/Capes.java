...
public class Capes {
    private static final String CAPE_OWNERS_URL = System.getProperty("client33.capes.ownersUrl");
    private static final String CAPES_URL = System.getProperty("client33.capes.listUrl");
    ...
    @PreInit(dependencies = Client33Executor.class)
    public static void init() {
        ...
        if (CAPE_OWNERS_URL == null || CAPES_URL == null) return;

        Client33Executor.execute(() -> {
            // Cape owners
            Stream<String> lines = Http.get(CAPE_OWNERS_URL)
                .exceptionHandler(e -> Client33.LOG.error("Could not load capes: {}",  e.getMessage()))
                .sendLines();
            if (lines != null) {
                lines.forEach(s -> {
        ...
        Client33.EVENT_BUS.subscribe(Capes.class);
    }
    ...
    private static class Cape {
        ...
        public Cape(String name) {
            this.identifier = Client33.identifier("capes/" + COUNT++);
            this.name = name;
        }
        ...
        public void download() {
            ...
            Client33Executor.execute(() -> {
                try {
                    ...
                    Client33.LOG.error("Failed to download cape '{}'", name, e);
                }
            });
        }
