...
package com.gaspoweredcake.client33.utils.entity;
...
public class EntityUtils {
    ...
    public static BlockPos getCityBlock(PlayerEntity player) {
        ...
        for (Direction direction : DirectionAccessor.client33$getHorizontal()) {
    ...
    public static boolean intersectsWithEntity(Box box, Predicate<Entity> predicate) {
        EntityLookup<Entity> entityLookup = ((WorldAccessor) mc.world).client33$getEntityLookup();
        ...
        if (entityLookup instanceof SimpleEntityLookup<Entity> simpleEntityLookup) {
            SectionedEntityCache<Entity> cache = ((SimpleEntityLookupAccessor) simpleEntityLookup).client33$getCache();
            LongSortedSet trackedPositions = ((SectionedEntityCacheAccessor) cache).client33$getTrackedPositions();
            Long2ObjectMap<EntityTrackingSection<Entity>> trackingSections = ((SectionedEntityCacheAccessor) cache).client33$getTrackingSections();
                ...
                while (longIterator.hasNext()) {
                    ...
                    if (s >= j && s <= m && t >= k && t <= n) {
                        ...
                        if (entityTrackingSection != null && entityTrackingSection.getStatus().shouldTrack()) {
                            for (Entity entity : ((EntityTrackingSectionAccessor) entityTrackingSection).<Entity>client33$getCollection()) {
